    ...
    <div className="mt-12 pt-8 border-t border-gray-200">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {displayedArticles.map((article) => (
          ...
          >
            <h3 className="font-medium text-gray-900 group-hover:text-accent-600 transition-colors mb-2 line-clamp-2">
              {article.title}
            </h3>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {article.sharedConcepts.slice(0, 3).map((concept) => (
                    <span
                      key={concept}
                      className="px-2 py-0.5 text-xs bg-accent-100 text-accent-700 rounded-full"
                    >
                      {concept}
                    </span>
                  ))}
